...
<s:url action="main" namespace="/po" var="mainURL"></s:url>
...
<s:form method="post" namespace="/po" action="main" theme="simple" cssClass="navbar-form">
	...
	<div role="main" class="container-fluid">
		...
		<table id="master" class="table table-striped table-hover table-list break-table">
			<tbody>
			<tr>
				<th class="col-md-1"><s:text name="fact.field.name" /></th><td class="col-md-3 fact_name"><c:out value="${form.fact.name}"/></td>
				<th class="col-md-1"><s:text name="purchase.field.fact_id" /></th><td class="col-md-3 fact_id"><c:out value="${form.factId}"/></td>
				<th class="col-md-1"><s:text name="purchase.field.purchase_date" /></th><td class="col-md-3"><s:textfield name="form.purchaseDate" theme="simple" size="10" maxlength="10" cssClass="form-control DateText purchase_date"/></td>
			</tr>
			<tr>
				<th class="col-md-1"><s:text name="fact.field.biz_no" /></th><td class="col-md-3 biz_no"><c:out value="${form.fact.bizNo}"/></td>
				<th class="col-md-1"><s:text name="fact.field.tel" /></th><td class="col-md-7 tel" colspan="3"><c:out value="${form.fact.tel}"/></td>
			</tr>
			<tr>
				<th class="col-md-1"><s:text name="fact.field.addr" /></th><td class="col-md-7 addr" colspan="3"><c:out value="${form.fact.addr}"/></td>
				<th class="col-md-1"><s:text name="purchase.field.master_id" /></th><td class="col-md-3"><c:out value="${form.masterId}"/></td>
			</tr>
			<tr>
				<th class="col-md-1"><s:text name="fact.field.memo" /></th><td class="col-md-7 fact_memo" colspan="3"><c:out value="${form.fact.memo}"/></td>
				<th class="col-md-1"><s:text name="purchase.field.invoice_nbr" /></th><td class="col-md-3"><s:textfield name="form.invoiceNbr" size="10" maxlength="10" maxcssClass="form-control form-control-fullwidth invoice_nbr"/></td>
			</tr>
			</tbody>
		</table>
		</div>
		...
		<div class="row" style="margin-top:20px;">
			<div class="col-xs-6"><h4><s:text name="purchase.message.detail"/></h4></div>
...
<s:iterator value="form.details" var="detail" status="idx">
				<tr>
					<td data-title="<s:text name="purchase.field.seq" />"><span class="form-control-static detail_seq">${idx.count}</span> <button title="" type="button" class="btn btn-danger remove show_tip" data-original-title="<s:text name="global.action.remove"/>"><i class="fa fa-trash-o"></i></button></td>
					...
					<td data-title="<s:text name="purchase.field.qty" />"><input name="qty" type="text" value="<c:out value='${detail.qty}'/>"  class="form-control form-control-fullwidth detail_qty" maxlength="6"/></td>
					<td data-title="<s:text name="prod.field.unit"/>"><p class="form-control-static detail_unit"><c:out value="${detail.prod.unit}"/></p></td>
					<td data-title="<s:text name="prod.field.price"/>"><input name="price" type="text" value="<c:out value='${detail.amt / detail.qty}'/>" class="form-control form-control-fullwidth detail_price" maxlength="6"/><input type="hidden" class="detail_price_orig" value="<c:out value='${detail.amt / detail.qty}'/>"/></td>
					<td data-title="<s:text name="purchase.field.detail_amt"/>"><p class="form-control-static detail_amt"><c:out value="${detail.amt}"/></p></td>
				</tr>
</s:iterator>
			</tbody>
		</table>
		...
		<table id="footer" class="table table-striped table-hover table-list break-table">
			<tbody>
			<tr>
				<th class="col-md-1"><s:text name="purchase.field.amt" /></th><td class="col-md-3 amt"></td>
				<th class="col-md-1"><s:text name="purchase.field.discount" /></th><td class="col-md-3"><s:textfield name="form.discount" cssClass="form-control form-control-fullwidth discount"/></td>
				<th class="col-md-1"><s:text name="purchase.field.total" /></th><td class="col-md-3 total"></td>
			</tr>
			<tr>
				<th class="col-md-1"><s:text name="purchase.field.memo" /></th><td class="col-md-11" colspan="5">
...
{id:"<c:out value="${fact.id}"/>",name:"<c:out value="${fact.name}"/>",biz_no:"<c:out value="${fact.bizNo}"/>",tel:"<c:out value="${fact.tel}"/>",addr:"<c:out value="${fact.addr}"/>",memo:"<c:out value="${fact.memo}"/>"},
</s:iterator>];
...
function addDetailRow() {
	...
	var row = '<tr>' + 
			'<td data-title="<s:text name="purchase.field.seq" />"><span class="form-control-static detail_seq">' + (len + 1) + '</span> <button title="" type="button" class="btn btn-danger remove show_tip" data-original-title="<s:text name="global.action.remove"/>"><i class="fa fa-trash-o"></i></button></td>' + 
			...
			'<td data-title="<s:text name="purchase.field.qty" />"><input name="qty" type="text" value="1" class="form-control form-control-fullwidth detail_qty" maxlength="6"/></td>' + 
			'<td data-title="<s:text name="prod.field.unit"/>"><p class="form-control-static detail_unit"></p></td>' + 
			'<td data-title="<s:text name="prod.field.price"/>"><input name="price" type="text" value="" class="form-control form-control-fullwidth detail_price" maxlength="6"/><input type="hidden" class="detail_price_orig"/></td>' + 
			'<td data-title="<s:text name="purchase.field.detail_amt"/>"><p class="form-control-static detail_amt"></p></td>';
	row += '</tr>';
...
function save(event) {
	...
	$('table#details tbody tr').each(function(i,e) {
		...
		if (price == '' || !isDecimal(price) || parseFloat(price) < 0) {
			alert('<s:text name="purchase.message.required.number"/>');
			error = true;
	...
	if (count <= 0) {
		alert('<s:text name="purchase.message.required.detail"/>');
	...
	if (!confirm('<s:text name="purchase.message.confirm.add"/>'))
		return false;
...
$(function () {
	...
	$(".fact").on('keydown.autocomplete', function() {
		$(this).autocomplete({
			...
			select: function( event, ui ) {
				...
				$('#factId').val(ui.item.value);
				$('.fact_name').text(ui.item.label);
				$('.fact_id').text(ui.item.value);
				$('.biz_no').text(ui.item.biz_no);
				$('.addr').text(ui.item.addr);
				$('.tel').text(ui.item.tel);
				$('.fact_memo').text(ui.item.memo);
	...
	$('table#details').on('keydown.autocomplete', '.detail_prod', function() {
		$(this).autocomplete({
			...
			source: function(request, response) {
				$.ajax({
				    type: "POST",
				    url: '<s:url action="main" namespace="/po" method="getProdList"/>',
				    dataType: "json",
				    data: {a: $('#factId').val(), b: request.term},
	...
	$('table#details').on('blur', '.detail_qty, .detail_price', function() {
		...
		if (qty == '' || price == '') {
			...
			if (!isDecimal(qty)) {
				alert('<s:text name="purchase.message.required.integer"/>');
				$(this).closest('tr').find('.detail_qty').val(1);
				return;
			}
